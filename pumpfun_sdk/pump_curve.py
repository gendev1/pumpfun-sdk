...
class BondingCurveState:
    ...
    def __repr__(self):
        return (
            f"BondingCurveState(virtualToken={self.virtual_token_reserves}, "
...
def calculate_bonding_curve_price(curve_state: BondingCurveState) -> float:
    ...
    token_price = (curve_state.virtual_sol_reserves / LAMPORTS_PER_SOL) / (
...
def calculate_output_amount(
    curve_state: BondingCurveState, input_amount: float, is_buy: bool = True
) -> float:
    """
    ...
    """
    if is_buy:
        virtual_sol = curve_state.virtual_sol_reserves + int(
            input_amount * LAMPORTS_PER_SOL
        )
        virtual_tokens = curve_state.virtual_token_reserves
    else:
        virtual_sol = curve_state.virtual_sol_reserves
        virtual_tokens = curve_state.virtual_token_reserves + int(
            input_amount * 10**TOKEN_DECIMALS
        )
